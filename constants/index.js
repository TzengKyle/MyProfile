export const projectInfos = [
  {
    ...
    type: '資料分析',
    intro: '透過研究對標企業的ESG報表，並與緯創軟體的ESG報表進行比較，找出可以增加員工留任率的可行策略。其中涉及將緯創軟體的數據進行「資料清洗」、「資料分析」、「報表呈現」以及「解決方案的發想與驗證」',
    h1s: ['簡介', '學習成果', '專案介紹'],
    h2s: [
      'ESG報表提取重點資訊',
      '數據分析',
    ...
    passages: [
      "在閱讀各家公司的報表後，我們初步了解了此次資料分析的目標，并在比較數據時展開了進一步的探索。在這個階段，我們尚未應用數據分析的技巧，而是更像是對整個問題框架進行了整理，并將其彙總為可以有效表達的簡報。",
      "在這個階段，我們將會實際運用Python或Excel對數據進行表格製作和圖表繪製。我們會先對資料進行適度清洗，提取出我們的重點。這包括檢查和處理缺失值、重複數據，以及解決任何其他數據質量問題。清洗後，我們將專注於我們感興趣的變量和指標，以便深入分析。",
      "在數據分析的過程中，我們將嘗試多種類型的圖表，以及不同的角度去展現我們從數據中得到的見解。這可能包括折線圖、柱狀圖、散點圖等，以便從不同的視覺化方式中找出潛在的模式或關聯性。通過將數據視覺化，我們可以更直觀地理解數據背後的故事，進而為未來的策略制定提供更有力的支持。"

    ],
    results: [
      '扎實的Python與程式語言能力',
      'Python資料處理套件與Excel樞紐分析表的應用',
      "接觸實際企業的數據",
      "與不同科系背景的成員合作完成完整的資料分析專案"
    ],
    iconUrls: ["/python.png", "/excel.png", "/matlab.png"],
    coverImgUrl: '/project5-img-1.png',
    imgIndexs: [[2, 3], [4, 5, 6], [7, 8]],
    pageUrl: '/posts/5',
    codeUrl: '',
    demoUrl: ''
  }
